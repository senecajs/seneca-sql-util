...
class QueryBuilder {
  ...
  static queryOfFrom(node) {
    Assert.strictEqual(node.whatami$, 'from_t')

    if (node.what$.whatami$ === 'table_t') {
      return QueryBuilder.queryOfTable(node.what$)
    }

    if (node.what$.whatami$ === 'select_t') {
      const from_q = QueryBuilder.queryOfSelect(node.what$)
  ...
  static queryOfSelect(node) {
    ...
    if (node.from$) {
      const from_q = QueryBuilder.queryOfFrom(node.from$)
