...
class QueryBuilder {
  static queryOfSelect(node) {
    ...
    const table_q = QueryBuilder.queryOfTable(node.from$)
    q.sql += ' from ' + table_q.sql
    q.values = q.values.concat(table_q.values)
  ...
  */
  ...
  static queryOfColumn(node) {
    ...
    if (null == node.alias$) {
      return makeResult({ sql: node.name$ }) // TODO: Escape the column name and the alias.
    }
  ...
  static queryOfTable(node) {
    ...
    if (node.alias$) {
      return makeResult({
        sql: [node.name$, node.alias$].join(' as ') // TODO: Escape the table name and the alias.
      })
    }

    return makeResult({ sql: node.name$ }) // TODO: Escape the table name.
  }
