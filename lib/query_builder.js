...
class QueryBuilder {
  static queryOfSelect(node) {
    ...
    const from_q = (() => {
      ...
      if (node.from$.whatami$ === 'select_t') {
        const from_q = QueryBuilder.queryOfSelect(node.from$)

        return makeResult({
          sql: parenthesized(from_q.sql),
          values: from_q.values
        })
      }
  ...
  */
  ...
  static queryOfColumn(node) {
    ...
    if (null == node.alias$) {
      return makeResult({ sql: escapedIdentifier(node.name$) }) // TODO: Escape the column name and the alias.
    }

    return makeResult({
      sql: [node.alias$, node.name$] // TODO: Escape the column name and the alias.
        .map(token => escapedIdentifier(token))
        .join('.')
    })
  }
  ...
  static queryOfTable(node) {
    ...
    return makeResult({ sql: escapedIdentifier(node.name$) }) // TODO: Escape the table name.
  }
