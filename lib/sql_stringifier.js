...
class SqlStringifer {
  static stringifySelect(ast) {
    ...
    if (null != ast.limit$) {
      Assert.strictEqual(ast.limit$.whatami$, 'limit_t')

      sql += ' limit ' + SqlStringifer.stringifyValue(ast.limit$.limit$)

      if (null != ast.limit$.offset$) {
        sql += ' offset ' + SqlStringifer.stringifyValue(ast.limit$.offset$)
      }
    }
  ...
  static stringifyValue(ast) {
    ...
    if (ast.alias$) {
      return [sql_value, doubleQuoted(ast.alias$)].join(' as ')
    }
